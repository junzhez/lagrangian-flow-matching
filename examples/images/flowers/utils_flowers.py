...
def generate_samples(model, parallel, savedir, step, net_="normal", img_size=128):
    ...
    with torch.no_grad():
        traj = node_.trajectory(
            torch.randn(64, 3, img_size, img_size, device=device),
            t_span=torch.linspace(0, 1, 100, device=device),
        )
        traj = traj[-1, :].view([-1, 3, img_size, img_size]).clip(-1, 1)
        traj = traj / 2 + 0.5
